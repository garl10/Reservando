...
Listado.prototype.buscarRestaurante = function (id) {
    ...
    var restaurantEncontrado = this.restaurantes.find(function (res){
        return res.id === id;
    });
...
//Funcion donde se agregan los elementos sin repetirse 

function agregarElementos(ele) {
    var elementosAgregados = ele.filter(function (elem, index, self) {
        return index === self.indexOf(elem);
    });

    return elementosAgregados.sort();
}
...
Listado.prototype.obtenerRubros = function () {
    var rubros = this.restaurantes.map(function (tipoDeComida) {
        return tipoDeComida.rubro;
    });

    return agregarElementos(rubros);
}
...
Listado.prototype.obtenerHorarios = function () {
    ...
    var arregloHorarios = this.restaurantes.map(function (horas) {
        return horas.horarios;
    });
    ...
    return agregarElementos(horarios);
}
